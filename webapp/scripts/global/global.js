(function($) {
    ...
    $.logout = function(sucFun) {
        $.ajax({
            type: 'GET',
            url: "/caa-personal-ws/ws/0.1/logout",
            dataType: "json",
            cache: false,
            async: true,
            success: function(data) {
    ...
    $.loadMailNum = function(userId) {
        $.ajax({
            type: 'GET',
            url: "/personal-ws/ws/0.1/inmail/num?state=0",
            dataType: "json",
            async: true,
            cache: false,
            success: function(data) {
    ...
    $.changeMailState = function(userId) {
        $.ajax({
            type: 'get',
            url: "/personal-ws/ws/0.1/change/inmail/state",
...
util = {
    setlogo: function() {
        ...
        $('.shop-official').attr('href', _resource.officialurl);
    },
    ...
    //2017-10-25
    tranferTime2: function(t, isyear, ishour) { //isyear ishour 为真时 不显示 年 不显示 小时
        if (!t) {
            return "";
        }
        var time = parseInt(t);
        ...
        var hourtxt = !ishour ? ("0000" + trans.getHours()).substring(("0000" + trans.getHours()).length - 2) + ":" +
            ("0000" + trans.getMinutes()).substring(("0000" + trans.getMinutes()).length - 2) + ":" +
            ("0000" + trans.getSeconds()).substring(("0000" + trans.getSeconds()).length - 2) : "";
        var result = yeartxt +
    ...
    formatMoney: function(money, unit) { //unit 表示带单位
        money = money + 0.0000001;
        var account = parseFloat(money);
        if (account >= 10000) {
            account = (account / 10000.00).toFixed(2) + "万";
        } else {
