...
//保存拍品相关的可变的数据  注意：竞价阶梯、起拍价都是可变的。
var auction_data = {
    meetId: util.getQueryString("meetId"), //获取拍品ID
...
var auctionHall_controller = {
    ...
    itemlist: function() {
        $.ajax({
            url: '/caa-search-ws/ws/0.1/lots/sort?start=0&count=5&meetId=' + auction_data.meetId,
            type: 'get',
            dataType: 'json',
            cache: false,
            async: false,
    ...
    auctionner: function() {
        $.ajax({
            url: '/caa-search-ws/ws/0.1/auctioneer?meetId=' + auction_data.meetId,
            type: 'get',
            dataType: 'json',
            cache: false,
            async: false,
    ...
    initLotInfo: function(lotId) {
        $.ajax({
            url: "/personal-ws/ws/0.1/lot/" + lotId,
            type: 'get',
            dataType: 'json',
            cache: false,
            async: false,
    ...
    latestBidInfo: function(lotId) { //最新的竞价信息 4种场景下调用改方法 每隔30秒调用一次  竞价成功调用一次  推送状态发生变化调用一次 0.00倒计时结束调用1次
        $.ajax({
            url: '/personal-ws/ws/0.1/lot/currentinfo/' + lotId,
            type: 'get',
            dataType: 'json',
            cache: false,
            async: false,
    ...
    checkDeposite: function(lotId) { //检测是否支付保证金
        $.ajax({
            url: "/personal-ws/ws/0.1/checkpay/lot/" + lotId,
            type: 'get',
            dataType: 'json',
            cache: false,
            async: false,
    ...
    loadJPH: function(lotId) { //加载竞买号
        $.ajax({
            url: '/personal-ws/ws/0.1/lot/' + lotId + '/mine',
            type: 'get',
            dataType: 'json',
            cache: false,
            async: false,
...
(function($) {
    ...
    $.fn.loadinfo = function(data) {
        ...
        $('#bid_name').html('<span>' + arr[data.lotStatus] + '</span><em style="margin-right: 16px">' + data.name + '</em>') //拍品名称
    ...
    $.fn.progressBidJoin = function(data) {
        ...
        display.find(".pay_bid").click(function() {
            ...
            $('#jph').html($('#show_jmh').html())
                //关闭出价弹窗
            $('.closeBtn').click(function() {
                $('.offerDiv').hide().closeDialog(); //拍品 当前价格;
    ...
    $.fn.finishBidCommon = function(data) {
        ...
        if (auction_data.lotMode == 1) {
            price = auction_data.amount * price;
        }
        var display = '<ul class="pm-bid-eyebrow end_time">' +
            ...
            '<p class="pp_endstate" style="padding-top: 25px">本拍品拍卖已结束</p>' +
    ...
    $.fn.timefinish = function() {
        ...
        var display =
            ...
            '<p class="pp_endstate" style="padding-top: 15px">本拍品倒计时已结束</p>' +
    ...
    $.fn.finishBidOwer = function(bidderNumber, bidCount) {
        this.find(".pm-bid").append('<p class="result">恭喜您，获得本拍品。<a target="_blank" href="/pages/lots/orderBid.html?showuse=1&lotId=' + auction_data.lotId + '#AuctConfirmation" class="colorred">查看成交确认书</a></p>');
        $('.pp_endstate').css({
            "padding-top": "15px"
        })
        return this;
    ...
    $.fn.finishBidOutter = function(data) {
        this.find(".pm-bid").append('<p class="result">很遗憾，您未获得获得本拍品。<a target="_blank" href="/pages/personal/home.html?showuse=3&lotId=' + auction_data.lotId + '" class="colorred">查看保证金</a></p>');
    ...
    $.fn.waitAuction = function(data) {
        ...
        $.each(data.items, function(index, data) {
            ...
            var li = '<li class="dis_block_parent" data-id=' + data.id + '>' +
                ...
                '<p class="p_td2 dis_block_child">自由竞买时间：<em>' + util.tranferTime4(data.freeTime) + '</em></p>' +
                ...
                '<p class="p_td2 dis_block_child">限时竞买时间：<em>' + util.tranferTime4(data.limitTime) + '</em></p>' +
                '<p class="p_td3 dis_block_child" style="width:90px;" id="all_count">拍品总量：<span id="lot_allnum"></span></p>' +
                '</div>' +
                '</div>' +
                '</li>'
            var $li = $(li).clone();
            ...
            if (data.status == 2) {
                $li.find('.viewstatus1').addClass('viewstatus').html('<p class="waiting_price waiting_priceone" style="padding-bottom: 20px;">本拍品已流拍</p>')
            } else if (data.status == 3) {
                var endprice = data.nowPrice;
                if (data.lotMode == 1) {
                    endprice = data.amount * data.nowPrice;
                }
                $li.find('.viewstatus1').addClass('viewstatus').html('<p class="waiting_price waiting_priceone">本拍品已成交</p><p class="waiting_price waiting_price_succ">成交价：<em>' + util.formatCurrency(endprice) + '元</em></p>')
            } else if (data.status == 4) {
                $li.find('.viewstatus1').addClass('viewstatus').html('<p class="waiting_price waiting_priceone">本拍品已中止</p><p class="waiting_price waiting_price_one">中止理由：<em>' + data.changeReason + '</em></p>')
            } else if (data.status == 5) {
                $li.find('.viewstatus1').addClass('viewstatus').html('<p class="waiting_price waiting_priceone">本拍品已撤拍</p><p class="waiting_price waiting_price_one">撤拍理由：<em>' + data.changeReason + '</em></p>')
            } else if (data.status == 6) {
                $li.find('.viewstatus1').addClass('viewstatus').html('<p class="waiting_price waiting_priceone">本拍品已暂缓</p>')
            } else if (data.status == 7) {
                $li.find('.viewstatus1').addClass('viewstatus').html('<p class="waiting_price waiting_priceone">本拍品已暂停</p>')
            }
            $ul.append($li)
        })
        $(".page-wrap").show().page({
