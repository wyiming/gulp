...
$(function() {
    ...
    function getUser() {
        var $user = $('#user');
        nowTimeL = (new Date()).getTime();
        $.getJSON("/caa-oc/ws/0.1/web/userinfo?time=" + nowTimeL, function(data) {
    ...
    function getMech() {
        ...
        nowTimeL = (new Date()).getTime();
        $.getJSON("/caa-oc/ws/0.1/web/corpauthinfo?time=" + nowTimeL, function(data) {
            bindState = data.bindState;
            ...
            $mechanism.find('[name="frMobile"]').val(data.frMobile).prev('span').html(data.frMobile);
        });
    }
    ...
    function editUser() {
        var $user = $('#user'),
            ...
            mobile = $user.find('[name="mobile"]').val().replaceSpace(),
        ...
        $.ajax({
            url: '/caa-oc/ws/0.1/web/editindivbidder',
    ...
    function editMech() {
        var $mechanism = $('#mechanism'),
            frName = $mechanism.find('[name="frName"]').val().replaceSpace(),
            frTel = $mechanism.find('[name="frTel"]').val().replaceSpace(),
        ...
        $.ajax({
            url: '/caa-oc/ws/0.1/web/editcorpbidder',
            type: 'post',
            contentType: "application/json",
            dataType: 'json',
            async: true,
    ...
    function getcorpcertinfo() {
        ...
        if (type != 1 && type != 1000) {
            var $userrealshow = $('#userAuthentication .user-real-show'),
                $userreal = $('#userreal'),
                $userreal2 = $('#userreal2');
            util.getdata('/caa-oc/ws/0.1/web/indivcertinfo', 'get', 'json', false, false, function(data) {
            ...
            util.getdata('/caa-oc/ws/0.1/web/corpcertinfo', 'get', 'json', false, false, function(data) {
                state = data.auditState;
                bindState = data.bindState;
                if (data.bindState != 1) {
                    $('.user-nobind-tel').show();
                ...
                if (state == 0) {
                    ...
                    $mechrealshow.find('.personnew_center  .personalitem').eq(6).find('span').html(data.frTel);
                    $mechrealshow.find('.personnew_center  .personalitem').eq(7).find('span').html(data.emails);
                } else if (state == 2) {
                    $('#mechreal').hide();
                    $mechrealshow.show();
                    $mechrealshow.find('#company_msg').show();
    ...
    var $userArtificial = $("#userreal2 .personalitem"),
        ...
        $userArtificialEmail = $userArtificial.eq(3),
        $userArtificialType = $userArtificial.eq(4),
        $userArtificialTnum = $userArtificial.eq(5),
        $userArtificialError = $userArtificial.eq(7),
    ...
    function getRealMech() {
        ...
        $.getJSON("/caa-oc/ws/0.1/web/corpauthinfo?time=" + nowTimeL, function(data) {
            ...
            var frMobile = data.frMobile + 1 + '';
            if (frMobile.indexOf('*') == -1) {
                $mechMobile.find('input').val(data.frMobile);
            }
    ...
    function checkcodeNum(uploading, company) {
        ...
        if ($(".personalitemtype select").val() == 1 || $(".personalitemtype select").val() == 0) {
            $(".uploadmind div").show();
            uploading.show();
            $('.picexample div').show();
        ...
        if ($(".personalitemtype select").val() == 2) {
            uploading.eq(0).show();
            uploading.eq(1).show();
        ...
        if ($(".personalitemtype select").val() == 3) {
            $(".uploadmind div").show();
            uploading.show();
            $('.picexample div').show();
        ...
        if ($(".personalitemtype select").val() == 4) {
            $(".uploadmind div").show();
            uploading.show();
            $('.picexample div').show();
            $(".uploadmind div").eq(0).html("上传通行证清晰正面照")
        ...
        if ($(".personalitemtype select").val() == 5) {
            index = 2;
            uploading.eq(0).show();
            uploading.eq(1).show();
            $('.picexample div').eq(0).show();
        ...
        if ($(".personalitemtype select").val() == 6) {
            uploading.eq(0).show();
            uploading.eq(1).show();
            $('.picexample div').eq(0).show();
            $('.picexample div').eq(1).show();
        ...
        if ($(".personalitemtype select").val() == 11) {
            uploading.eq(0).show();
            uploading.eq(1).show();
            $('.picexample div').eq(0).show();
            $('.picexample div').eq(1).show();
        ...
        if ($(".personalitemType select").val() == 7 || $(".personalitemType select").val() == 8 || $(".personalitemType select").val() == 9 || $(".personalitemType select").val() == 10) {
            company.show();
            $('.picexample div').show();
    ...
    function loadMail(dNum) {
        ...
        $.ajax({
            url: '/personal-ws/ws/0.1/inmail/mine?state=&pagesize=10&pageindex=' + num,
            type: 'get',
            dataType: 'json',
            cache: false,
            async: true,
    ...
    var bulidAuction = function(data, deposit, mylot) {
        ...
        if (data.items.length == 0) {
            ...
            $.each(data.items, function(index, item) {
                ...
                if (item.meetType == '0') {
                    htmlStr += '<td class="img"><a href="/pages/lots/orderBid.html?lotId=' + item.lotId + '" target="_blank"><img src="' + imgurl + '"> <span>' + shortName + '</span></span></td> ';
                } else {
                    htmlStr += '<td class="img"><a href="/pages/lots/item.html?lotId=' + item.lotId + '" target="_blank"><img src="' + imgurl + '"> <span>' + shortName + '</span></span></td> ';
                }
                ...
                if (deposit == 'true') {
                    ...
                    var classname = (item.lotStatus == 1 || item.lotStatus == 7) ? 'start' : '';
                    var statustext = item.lotStatus ? lotStatusText[item.lotStatus] : '';
                    if (item.subStatus == '0') {
                        statustext = '等待拍卖师操作';
                    }
                ...
                if (deposit == 'true') {
                    ...
                    htmlStr += '<td class="button"><input type="button" class="look" value="查看拍品" data-type="' + item.meetType + '" data-lotid="' + item.lotId + '">';
                }
    ...
    $('#mine-auction .person_auction').on('click', '.look', function() {
        var lotId = $(this).data('lotid'),
            type = $(this).data('type');
        if (type == 0) {
            window.open('/pages/lots/orderBid.html?lotId=' + lotId);
        } else if (type == 1) {
            window.open('/pages/lots/item.html?lotId=' + lotId);
        }
    });
    //查看保证金
    ...
    function getAuction(pageindex) {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.totalCount == 0) {
                    $panel.parent().hide().prev().show(); //没有拍品时 显示 无拍品提示图片
                } else {
    ...
    function checkUser() {
        ...
        if (!mobile || mobile.length == 0) {
            $userArtificialMobile.find('em').eq(1).html('手机号不能为空').show();
            userflag = false;
        }
    ...
    function uploadimgtwo(formone, formtwo, idp, jsonData) {
        ...
        if (type == 1) {
            if (browser != "Microsoft Internet Explorer" && trim_Version != "MSIE8.0") {
                ...
                if (ind.length < 1) {
                    $(".picerror-msg").show().delay(3000).hide(1).delay(3000).hide(1);
                    return false
                }
            } else {
                if ($(".preview-ie-input").eq(0).val() == "" || $(".preview-ie-input").eq(1).val() == "") {
                    $(".picerror-msg").show().delay(3000).hide(1);
                    return false
                }
            }
        } else {
    ...
    function uploadimgthree(formone, formtwo, formthree, idp, jsonData) {
        ...
        if (browser != "Microsoft Internet Explorer" && trim_Version != "MSIE8.0") {
            ...
            if ($(".preview-ie-input").eq(0).val() == "" || $(".preview-ie-input").eq(1).val() == "" || $(".preview-ie-input").eq(2).val() == "") {
                $(".picerror-msg").show().delay(3000).hide(1);
                return false
            }
        }
    ...
    function submitUser(jsonData) {
        $.ajax({
            url: '/caa-oc/ws/0.1/web/indivauth',
            type: 'post',
            dataType: 'text/plain',
            async: true,
            data: jsonData,
            ...
            error: function(xhr) {
                if (xhr.status == 200) {
                    ...
                    var info = JSON.parse(xhr.responseText);
                    $('#phone_error_tip').html(info.error).show();
                }
            }
        })
    }
    ...
    function checkother() {
        var corpName = $mechCropname.find('input').val().replaceSpace(),
            frName = $mechFrname.find('input').val().replaceSpace(),
            idType = $mechType.find('select').val(),
            frIdNum = $mechTnum.find('input').val().replaceSpace(),
            frTel = $mechTel.find('input').val().replaceSpace(),
            frMobile = $mechMobile.find('input').val().replaceSpace(),
            emails = $mechEmail.find('input').val(),
            businessNumber = $mechBnumber.find('input').val().replaceSpace();
        ...
        if (!frMobile || frMobile.length == 0) {
            $mechMobile.find('em').eq(1).html('法人手机不能为空').show();
            flagmech = false;
        } else if (!(yzmobile.test(frMobile))) {
            $mechMobile.find('em').eq(1).html('法人手机格式错误').show();
            flagmech = false;
        }
        ...
        if (browser == "Microsoft Internet Explorer" && trim_Version == "MSIE8.0") {
            ...
            formcompany($('#permitphoto-ie1'), $('#permitphoto-ie'), $('#permitphoto-ie-one'), $('#permitphoto-ie-two'), jsonData_ie)
        } else {
            var jsonData = {
                corpName: corpName,
                frName: frName,
        ...
        function formcompany(formone, formtwo, formthree, formfour, jsonData) {
            if (browser != "Microsoft Internet Explorer" && trim_Version != "MSIE8.0") {
                ...
                if ($('#certificates1').val() == "" || $('#certificates1').val() == undefined) {
                    $(".picerror-msg").eq(0).show();
                    return
                }
                if (ind.length < 3) {
                    $(".picerror-msg").eq(1).show();
                    return false
                }
            } else {
                if ($('#certificates2-ie').val() == "" || $('#certificates2-ie').val() == undefined) {
                    $(".picerror-msg").eq(0).show();
                    return
                }
                if ($(".preview-ie-input").eq(0).val() == "" || $(".preview-ie-input").eq(1).val() == "" || $(".preview-ie-input").eq(2).val() == "") {
    ...
    function submitcompany(jsonData) {
        $.ajax({
            url: '/caa-oc/ws/0.1/web/corpauth',
            type: 'post',
            dataType: 'json',
            async: true,
            data: jsonData,
    ...
    var $usereal = $("#userreal .personalitem"),
        ...
        $userRealBank = $usereal.eq(2),
        $userRealMobile = $usereal.eq(3),
        $userRealMsg = $usereal.eq(4);
    ...
    $(document).on('keyup', '.yz-idnum', function() {
        var idnum = $(this).val();
        if (idnum == null || idnum.length < 10 || !/(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/.test(idnum)) {
            var err = $(this).next('em');
    ...
    $(document).on('keyup', '.yz-emails', function() {
        var emails = $(this).val();
        if (emails == null || emails.length < 10 || !/^(\w-*\.*)+@(\w-?)+(\.\w{2,})+$/.test(emails)) {
            var err = $(this).next('em');
    ...
    $('#userreal .submit').click(function() {
        ...
        var name = $userRealName.find('input').val().replaceSpace(),
            idNum = $userRealNum.find('input').val().replaceSpace(),
            bankCard = $userRealBank.find('input').val().replaceSpace(),
            mobile = $userRealMobile.find('input').val().replaceSpace(),
        ...
        if (!bankCard || bankCard.length == 0) {
            $userRealBank.find('em').html('银行卡号不能为空').show();
            msgflag = false;
        }

        if (!mobile || mobile.length == 0) {
            $userRealMobile.find('em').html('手机号码不能为空').show();
            msgflag = false;
        } else if (!(yzmobile.test(mobile)) || !mobile) {
            $userRealMobile.find('em').html('请输入正确的手机号码').show();
            msgflag = false;
        }
        ...
        $.ajax({
            url: '/caa-personal-ws/ws/0.1/bankcard/security/auth',
            type: 'post',
            contentType: "application/json",
            async: true,
            dataType: 'json',
            ...
            success: function(data) {
                ...
                if (data.code == '200' || data.code == '09015' || data.code == '01035' || data.code == '01005' || data.code == '09011' || data.code == '08022') {
                    renzhengflag = false;
                    getcorpcertinfo();
                } else {
    ...
    $('.getcode').click(function() {
        if ($(this).hasClass('tosend')) {
            ...
            localStorage.time = 0;
            $.ajax({
                url: '/caa-oc/ws/0.1/bidder/security/sms?type=1',
                type: 'get',
                dataType: 'json',
                cache: false,
                async: true,
    ...
    $('.get-bind-code').click(function() {
        if ($(this).hasClass('tosend')) {
            ...
            var mobile = $('.bind-user-tel').val().replaceSpace();
