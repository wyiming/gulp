...
var order_bid = {
    lotId: util.getQueryString("lotId"), //获取拍品ID
...
var order_bid_controller = {
    ...
    initLotInfo: function() {
        $.ajax({
            url: "/personal-ws/ws/0.1/lot/" + order_bid.lotId,
            type: 'get',
            dataType: 'json',
            cache: false,
            async: false,
    ...
    latestBidInfo: function() { //最新的竞价信息 4种场景下调用改方法 每隔30秒调用一次  竞价成功调用一次  推送状态发生变化调用一次 0.00倒计时结束调用1次
        $.ajax({
            url: '/personal-ws/ws/0.1/lot/currentinfo/' + order_bid.lotId,
            type: 'get',
            dataType: 'json',
            cache: false,
            async: false,
    ...
    checkDeposite: function() { //检测是否支付保证金
        $.ajax({
            url: "/personal-ws/ws/0.1/checkpay/lot/" + order_bid.lotId,
            type: 'get',
            dataType: 'json',
            cache: false,
            async: false,
    ...
    loadJPH: function() { //加载竞买号
        $.ajax({
            url: '/personal-ws/ws/0.1/lot/' + order_bid.lotId + '/mine',
            type: 'get',
            dataType: 'json',
            cache: false,
            async: false,
    ...
    detailinit: function() {
        ...
        util.getdata('/caa-search-ws/ws/0.1/lot/' + order_bid.lotId + '/introduction/', 'get', 'json', false, true, function(data) {
            ...
            $('#NoticeDetail .detail-common-text').html(data.content); //拍品公告
            $('#bigpicshow img').attr("src", data.pics[0])
            if (!data.position) {
                $('#position').addClass('hidden')
            }
            $('#position').html('拍品所在地：' + data.position);
            var pics = '';
...
(function($) {
    ...
    $.fn.finishBidCommon = function(data) {
        ...
        var display = '<ul class="pm-bid-eyebrow end_time">' +
            ...
            '<p class="pp_endstate" style="padding-top: 20px">本拍品拍卖已结束</p>' +
    ...
    $.fn.finishBidOwer = function(bidderNumber, bidCount) {
        this.find(".pm-bid").append('<p class="result">恭喜您，获得本拍品。<a href="#RecordContent" class="colorred success_book">查看成交确认书</a></p>');
    ...
    $.fn.finishBidOutter = function(data) {
        this.find(".pm-bid").append('<p class="result">很遗憾，您未获得获得本拍品。<a href="/pages/personal/home.html?showuse=3&lotId=' + order_bid.lotId + '" class="colorred">查看保证金</a></p>');
        var mydiv = $('<div class="my-info clearfix"></div>');
        if (parseFloat(order_bid.meetStatus) > 1) {
            $('.hallenter').hide()
            $('#J_finish_result').css({
