...
$(function() {
    ...
    $('.submit').click(function() {
        ...
        if (submitBol) {
            ...
            json.idNum = $(".card-num").val().replaceSpace();
            json.name = $(".card-name").val().replaceSpace();
            json.mobile = $("#user_mobile").val().replaceSpace();
            ...
            $.ajax({
                ...
                error: function(data) {
                    ...
                    if (status.error == "您输入的证件号码与实名认证资料不符") {
                        $(".card-error").show();
                        $(".card-error em").html("您输入的证件号码与实名认证资料不符")
                    } else {
                        $(".card-error").hide();
                    }
                    if (status.error == "您输入的手机号码与实名认证资料不符") {
                        $(".tel-error").show();
                        $(".tel-error em").html("您输入的手机号码与实名认证资料不符")
                    } else {
                        $(".tel-error").hide();
                    }
                    if (status.error == "您输入的竞买人姓名与实名认证资料不符") {
                        ...
                        if (myApp.papersType != 7) {
                            $(".name-error em").html("您输入的竞买人姓名与实名认证资料不符")
                        } else {
                            $(".name-error em").html("您输入的机构名称与实名认证资料不符")
                        }
                    } else {
                        $(".name-error").hide();
                    }
    ...
    if (navigator.userAgent.indexOf("Trident") == -1) {
        if (user.type() == 1) {
            ...
            $("#user_mobile").attr("placeholder", "实名认证的手机号");
        } else {
...
function loadPayInfo(lotId) {
    $.ajax({
        url: '/personal-ws/ws/0.1/apply/lot/' + lotId,
        type: 'get',
        dataType: 'json',
        cache: false,
        async: true,
...
function checkAgree() {
    //判断是否勾选协议
    $('.agree .fields input[type="checkbox"]').each(function() {
        if (!$(this).is(':checked')) {
            $(this).siblings('span').show();
            $('.submit-content input').addClass('disabled');
            return false;
