    ...
    var appendauctioncontent = function(data, liststyle) {
        ...
        if (liststyle && liststyle == "liststyle") {
            ...
            $li = '<li class="other-meet">' +
                ...
                '<div class="meet-btn"><a target="_blank" class="meeting-btn lotlist">拍品目录</a><a target="_blank" class="meeting-btn go-meeting back-red">拍卖大厅</a></div>' +
                '</li>';
        } else {
            $li = '<li class="index-meet">' +
                '<div class="img-wrap"><img src="/themes/images/meeting.jpg" alt="">' +
                '<div class="other-wrap"><p class="meeting-type">专业拍</p><p class="meetingcount">123次围观</p></div>' +
                '</div>' +
                '<div class="meeting-info">' +
                ...
                '<div class="meet-btn"><a target="_blank" class="meeting-btn lotlist">拍品目录</a><a target="_blank" class="meeting-btn go-meeting back-red">拍卖大厅</a></div></div>' +
                // '</a>' +
                '</li>';
        }
        var $ul = $("<ul></ul>");
        ...
        for (var i = 0; i < auclen; i++) {
            ...
            $lichild.attr("id", $data.id).addClass(active)
                ...
                .find(".go-meeting").attr("href", "/pages/meeting/hall.html?meetId=" + $data.id).end()
            ...
            if ($data.mode == 1) {
                $lichild.find(".go-meeting").remove();
            }
            $ul.append($lichild);
        }
...
AUCTIONNUMBER = {
    ...
    createMeetinglist: function(urlsting, $container, $this, liststyle) {
        ...
        var getdata = { url: "/caa-search-ws/ws/0.1/auctionmeets?start=0&count=" + countnum + urlsting };
        //status 1 正在进行 2 已完成的拍卖会 0即将开始 不设置就是不限
