$.fn.lot = function(options) {
    ...
    var buildLotlist = {
        ...
        buildOneLot: function(self, data, type) {
            ...
            if (data.meetType == 0) {
                self.find('.link-wrap').attr("href", "/pages/lots/orderBid.html?lotId=" + data.id);
            } else if (data.meetType == 1) {
                self.find('.link-wrap').attr("href", "/pages/lots/item.html?lotId=" + data.id);
            }
            if (data.pic[0]) {
            ...
            if (status == 0) {
                ...
                currentPrice = _nowPrice > 0 ? _nowPrice : _startPrice;
                if (data.lotMode == 1) {
                    currentPrice = data.amount * currentPrice;
                }
                currentPriceTxt = buildLotlist.lotTxtArr[1]
            } else if (status == 2) {
                currentPrice = _startPrice;
                currentPriceTxt = buildLotlist.lotTxtArr[8]
            ...
            self.find('.info-section').find("p")
                .eq(0).addClass(buildLotlist.moneyClass[status]).find('span[class=label]').text(currentPriceTxt).end().find('.lot-current-price').html(util.formatMoney(currentPrice) + '元' + _unit).end().end()
